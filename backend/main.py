...
ВИПРАВЛЕНА версія з відновленою функціональністю та виправленими undefined проблемами
...
def safe_register_quests_routes(app):
    ...
    # Останній fallback - fallback маршрути
    logger.warning("⚠️ Використовуємо fallback маршрути")
    register_fallback_routes(app)
    return True
...
def register_api_routes(app):
    ...
    # Реєстрація маршрутів авторизації (КРИТИЧНО ВАЖЛИВО)
    try:
        ...
        # НЕ підіймаємо помилку, щоб сервер міг запуститися

    # Реєстрація маршрутів користувачів (КРИТИЧНО ВАЖЛИВО)
    try:
