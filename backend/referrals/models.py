...
class RewardStatus(Enum):
    """Статуси реферальних винагород"""
    PENDING = "pending"       # Винагорода очікує зарахування
    PAID = "paid"             # Винагорода зарахована
    FAILED = "failed"         # Винагорода не зарахована
    CANCELLED = "cancelled"   # Винагорода скасована


class ReferralLevel(Enum):
    """Рівні реферальної програми"""
    LEVEL_1 = 1       # Перший рівень (безпосередні реферали)
    LEVEL_2 = 2       # Другий рівень (реферали рефералів)
...
class Referral:
    ...
    Attributes:
        ...
        status (RewardStatus): Статус реферальної винагороди
        reward_amount (float): Сума винагороди за реферала
        level (ReferralLevel): Рівень реферальної програми
    ...
    def __init__(
            ...
            reward_amount=50.0,
            level=ReferralLevel.LEVEL_1.value,
            id=None,
            completed_at=None,
            reward_paid_at=None
    ):
    ...
    def from_dict(cls, data):
        ...
        referral = cls(
            ...
            status=data.get("status", RewardStatus.PENDING.value),
            reward_amount=data.get("reward_amount", 50.0),
            level=data.get("level", ReferralLevel.LEVEL_1.value),
            id=data.get("id"),
        )
    ...
    def pay_reward(self):
        ...
        Returns:
            Referral: Оновлений об'єкт реферального запису
        """
        self.status = RewardStatus.PAID.value
    ...
    def is_paid(self):
        ...
        """
        return self.status == RewardStatus.PAID.value and self.reward_paid_at is not None
    ...
    def cancel(self):
        ...
        Returns:
            Referral: Оновлений об'єкт реферального запису
        """
        self.status = RewardStatus.CANCELLED.value
        return self
    ...
    def mark_as_failed(self):
        ...
        Returns:
            Referral: Оновлений об'єкт реферального запису
        """
        self.status = RewardStatus.FAILED.value
        return self
