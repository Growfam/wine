...
// ЧИТАЄМО RESPONSE ТІЛЬКИ ОДИН РАЗ!!!
let response;
let responseText = '';

// Спочатку читаємо body
try {
    const contentType = fetchResponse.headers.get('content-type');

    if (contentType && contentType.includes('application/json')) {
        // Якщо JSON - читаємо як JSON
        response = await fetchResponse.json();
    } else {
        // Якщо не JSON - читаємо як текст
...
// Тепер можемо перевіряти статуси і кидати помилки
...
// Перевірка статусів БЕЗ повторного читання body!
if (!fetchResponse.ok) {
    console.error(`❌ API Error response:`, response);

    // Спеціальна обробка для помилок авторизації
    if (safeIncludes(url, 'auth/telegram')) {
        const errorMsg = response.message || response.error || fetchResponse.statusText;
        throw new Error(`Помилка авторизації: ${fetchResponse.status} - ${errorMsg}`);
    }
}
...
// Фінальна перевірка - якщо все ще не ok
if (!fetchResponse.ok) {
    const errorMessage = response.message || response.error || `Помилка серверу: ${fetchResponse.status}`;
