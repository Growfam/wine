...
(function () {
  ...
  async function refreshToken() {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/${normalizeEndpoint(API_PATHS.AUTH.REFRESH_TOKEN)}`,
        {
          method: 'POST',
          headers: {
  ...
  async function apiRequest(endpoint, method = 'GET', data = null, options = {}) {
    try {
      ...
      if (safeIncludes(endpoint, 'http')) {
        ...
        url = `${API_BASE_URL}/${normalizedEndpoint}`.replace(/([^:]\/)\/+/g, '$1'); // Видаляємо зайві послідовні слеші
  ...
  function showError(message, type = 'error') {
    ...
    let errorMessage =
      typeof message === 'string' ? message : message.message || 'Сталася невідома помилка';
