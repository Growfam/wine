...
const WinixWallet = {
  ...
  sendTokens: function (recipientId, amount, note) {
    return new Promise((resolve, reject) => {
      ...
      if (window.WinixAPI && typeof window.WinixAPI.apiRequest === 'function') {
        window.WinixAPI.apiRequest('/api/send_tokens', 'POST', data).then(resolve).catch(reject);
        return;
      }
  ...
  filterTransactions: function (filter) {
    ...
    this.fetchTransactionsWithFilter(100, filter)
      .then((transactions) => {
        this.hideLoading();
  ...
  loadTransactions: function (silent = false, limit = this.config.transactionsLimit) {
    ...
    const tryFetchTransactions = () => {
      return this.fetchTransactions(limit)
        ...
        .catch((error) => {
          console.warn(
            `Спроба ${retryCount + 1}/${maxRetries} завантаження транзакцій не вдалася:`,
            error
          );
          ...
          if (retryCount < maxRetries) {
            ...
            return new Promise((resolve) =>
              setTimeout(() => resolve(tryFetchTransactions()), delay)
            );
          } else {
            // Всі спроби не вдалися, використовуємо локальне сховище
            console.error(
              'Всі спроби завантаження транзакцій не вдалися, використовуємо локальні дані'
            );
            if (!silent) {
              this.hideLoading();
            }
    ...
    this.state.transactions = [
      {
        ...
        created_at: new Date(
          now.getTime() - (Math.floor(Math.random() * 7) + 1) * 24 * 60 * 60 * 1000
        ).toISOString(),
        status: 'completed',
      },
      {
      ...
      },
      {
        ...
        created_at: new Date(
          now.getTime() - (Math.floor(Math.random() * 5) + 1) * 24 * 60 * 60 * 1000
        ).toISOString(),
        status: 'completed',
      },
      {
        ...
        created_at: new Date(
          now.getTime() - (Math.floor(Math.random() * 2) + 1) * 24 * 60 * 60 * 1000
        ).toISOString(),
        status: 'completed',
      },
    ];
  ...
  updateTransactionsList: function (isHistoryList = false, useFilteredData = false) {
    ...
    transactions.forEach((transaction, index) => {
      ...
      date.textContent = this.formatDate(
        transaction.created_at || transaction.timestamp || transaction.date
      );
