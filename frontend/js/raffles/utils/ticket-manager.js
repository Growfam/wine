...
(function () {
  ...
  if (typeof window.WinixRaffles === 'undefined') {
    console.error(
      '❌ WinixRaffles не знайдено! Переконайтеся, що core.js підключено раніше ticket-manager.js'
    );
    return;
  }
  ...
  const ticketManager = {
    ...
    getUserCoins: function () {
      ...
      try {
        ...
        if (lastTxData) {
          ...
          if (
            lastTx.timestamp &&
            now - lastTx.timestamp < 60000 &&
            lastTx.newBalance !== undefined
          ) {
      ...
      if (window.WinixRaffles && window.WinixRaffles.participation) {
        ...
        if (participation.participatingRaffles) {
          participation.participatingRaffles.forEach((raffleId) => {
            ...
            const ticketCount =
              (participation.userRaffleTickets && participation.userRaffleTickets[raffleId]) || 1;
    ...
    participateInRaffle: async function (raffleId, entryCount = 1) {
      try {
        ...
        const result = await window.WinixRaffles.participation.participateInRaffle(
          raffleId,
          'delegate',
          entryCount
        );
