...
(function() {
     ...
     */
    function setCache(type, key, data, ttl) {
        try {
     ...
     */
    function getCache(type, key, defaultValue = null) {
        try {
            // Якщо тип невалідний, використовуємо GLOBAL
            const cacheType = CACHE_TYPES[type] ? type : 'GLOBAL';
     ...
     */
    function hasValidCache(type, key) {
        try {
            // Якщо тип невалідний, використовуємо GLOBAL
            const cacheType = CACHE_TYPES[type] ? type : 'GLOBAL';
     ...
     * @returns {boolean} Успішність операції
     */
    function clearCacheByType(type = null) {
        try {
     ...
     * @returns {boolean} Успішність операції
     */
    function updateCacheTTL(type, key, ttl) {
        try {
            // Якщо тип невалідний, використовуємо GLOBAL
            const cacheType = CACHE_TYPES[type] ? type : 'GLOBAL';
    ...
    // Автоматичне очищення кешу при низькому заряді або перед вивантаженням сторінки
    window.addEventListener('beforeunload', function() {
        // Тільки якщо багато записів у кеші
        if (getCacheStats().storage.cacheItems > 50) {
            cleanupCache();
        }
    });
