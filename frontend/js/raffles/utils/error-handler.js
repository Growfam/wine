...
(function () {
  ...
  if (typeof window.WinixRaffles === 'undefined') {
    console.error(
      '❌ WinixRaffles не знайдено! Переконайтеся, що core.js підключено раніше error-handler.js'
    );
    return;
  }
  ...
  const errorHandler = {
    ...
    handleJavaScriptError: function (event) {
      ...
      if (this.isCriticalError(event.message)) {
        ...
        if (
          this.config.autoReloadOnCritical &&
          this.stats.errorsInSession >= this.config.maxErrorsBeforeReload
        ) {
    ...
    handlePromiseError: function (event) {
      ...
      if (
        message &&
        (message.toLowerCase().includes('завершено') ||
      ...
      if (
        message &&
        (message.toLowerCase().includes('недостатньо') || message.toLowerCase().includes('жетон'))
      ) {
        this.showUserFriendlyError(
          'Недостатньо жетонів для участі. Отримайте більше жетонів.',
          'warning'
        );
        event.preventDefault();
        return;
    ...
    handleHttpError: function (response, url) {
      ...
      if (response.status === 404) {
        ...
        this.showUserFriendlyError(
          'Забагато запитів. Зачекайте кілька секунд і спробуйте знову.',
          'warning'
        );
    ...
    handleFetchError: function (error, url) {
      ...
      // Показуємо користувацьке повідомлення
      this.showUserFriendlyError(
        "Проблеми з підключенням. Перевірте інтернет-з'єднання.",
        'warning'
      );
    ...
    handleRaffleParticipationError: function (error, raffleId) {
      ...
      if (error.message && error.message.toLowerCase().includes('жетон')) {
        ...
        this.showUserFriendlyError(
          'Недостатньо жетонів для участі. Отримайте більше жетонів і спробуйте знову.',
          'warning'
        );
      } else if (
        error.message &&
        ...
        this.showUserFriendlyError(
          'Будь ласка, зачекайте кілька секунд перед наступною спробою.',
          'info'
        );
      } else {
        // Інші помилки
        this.showUserFriendlyError(
          'Не вдалося взяти участь у розіграші. Спробуйте пізніше.',
          'warning'
        );
      }
    ...
    handleRaffleError: function (error) {
      ...
      if (error.message && error.message.toLowerCase().includes('uuid')) {
        this.showUserFriendlyError(
          'Дані розіграшу недійсні. Спробуйте оновити сторінку.',
          'warning'
        );
        return;
      }
      ...
      // Стандартна обробка
      this.showUserFriendlyError(
        'Помилка при роботі з розіграшем. Спробуйте оновити сторінку.',
        'warning'
      );
    },
    ...
    getHumanReadableMessage: function (errorMessage) {
      ...
      if (
        errorMessage.toLowerCase().includes('розіграш') &&
    ...
    resetParticipationState: function () {
      ...
      if (window.WinixRaffles && window.WinixRaffles.participation) {
        ...
        if (
          window.WinixRaffles.participation.activeTransactions &&
      ...
      const processingButtons = document.querySelectorAll(
    ...
    markRaffleAsInvalid: function (raffleId) {
      ...
      if (window.WinixRaffles) {
        ...
        if (
          window.WinixRaffles.participation &&
          window.WinixRaffles.participation.invalidRaffleIds
        ) {
