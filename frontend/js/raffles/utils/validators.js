...
(function () {
  ...
  if (typeof window.WinixRaffles === 'undefined') {
    console.error(
      '❌ WinixRaffles не знайдено! Переконайтеся, що core.js підключено раніше validators.js'
    );
    return;
  }
  ...
  const validators = {
    ...
    isUserParticipating: function (raffleId) {
      ...
      if (Array.isArray(WinixRaffles.state.userRaffles)) {
        return WinixRaffles.state.userRaffles.some(
          (raffle) => raffle.raffle_id === raffleId || raffle.id === raffleId
        );
      }
    ...
    isRaffleCompleted: function (raffleId, pastRaffles) {
      ...
      return pastRaffles.some((raffle) => raffle.raffle_id === raffleId || raffle.id === raffleId);
    },
    ...
    isParticipationInProgress: function () {
      return (WinixRaffles.participation && WinixRaffles.participation.requestInProgress) === true;
    },
    ...
    findRaffleById: function (raffleId) {
      ...
      if (WinixRaffles.history && Array.isArray(WinixRaffles.history.historyData)) {
        raffle = WinixRaffles.history.historyData.find(
          (r) => r.id === raffleId || r.raffle_id === raffleId
        );
        if (raffle) return raffle;
      }
