...
(function () {
  ...
  if (typeof window.WinixRaffles === 'undefined') {
    console.error(
      '❌ WinixRaffles не знайдено! Переконайтеся, що core.js підключено раніше sync-service.js'
    );
    return;
  }
  ...
  const syncService = {
    ...
    syncBalance: function (force = false) {
      ...
      if (
        hasWinixCore() &&
        window.WinixCore.isBalanceUpdateLocked &&
        window.WinixCore.isBalanceUpdateLocked()
      ) {
        if (!force) {
    ...
    _legacyUpdateBalanceData: function (data) {
      if (!data) return;
      console.log(
        '⚠️ Використовується застарілий метод оновлення балансу. Рекомендуємо оновити до WinixCore.'
      );
      ...
      if (data.coins !== undefined) {
        ...
        if (oldCoins !== newCoins) {
          console.log(
            `💰 sync-service: Оновлення балансу жетонів: ${oldCoins} -> ${newCoins}, різниця: ${newCoins - oldCoins}`
          );
        }
