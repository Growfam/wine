...
(function() {
        ...
        return new Promise((resolve, reject) => {
            ...
            function checkAPI() {
                ...
                if (attempts >= maxAttempts) {
                    console.error('❌ WinixAPI не знайдено після кількох спроб! Намагаємося продовжити без нього.');
                    resolve(false);
                    return;
                }
    ...
    async function initWinixRaffles() {
        ...
        // Якщо об'єкт вже існує, оновлюємо налаштування
            ...
            if (window.WinixRaffles.config.autoRefreshInterval === 120000) { // Якщо стандартне значення
    ...
    initWinixRaffles()
        .then(rafflesModule => {
            if (rafflesModule.state.isInitialized) {
                console.log('✅ WinixRaffles вже ініціалізовано');
            } else {
                // Ініціалізуємо модуль, якщо він не був ініціалізований
                rafflesModule.init();
            }
        })
        .catch(error => {
            console.error('❌ Помилка ініціалізації WinixRaffles:', error);
            ...
            if (typeof window.WinixRaffles === 'undefined') {
                ...
                // Спроба ініціалізації базової версії
