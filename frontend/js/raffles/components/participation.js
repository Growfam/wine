    ...
    const participation = {
        ...
        loadUserRaffles: async function(forceRefresh = false) {
            ...
            try {
                ...
                // Запит до API
                const response = await window.WinixAPI.apiRequest(`user/${userId}/raffles?nocache=${now}`, 'GET', null, {
                    suppressErrors: true,
                    hideLoader: true,
                    timeout: 10000,
                    allowParallel: true,
                ...
                if (response && response.status === 'success' && Array.isArray(response.data)) {
                    ...
                    console.warn(`⚠️ Помилка завантаження розіграшів: ${response?.message || 'Невідома помилка'}`);
                    return {
                        success: false,
            ...
            try {
                ...
                buttons.forEach(button => {
                    ...
                    const isInvalid = invalidCache[raffleId] ||
                                   (window.WinixRaffles && window.WinixRaffles.state &&
        ...
        participateInRaffle: async function(raffleId, raffleType, entryCount = 1) {
            ...
            try {
                ...
                // Обробка відповіді
                console.log(`📩 Отримано відповідь на запит участі:`,
