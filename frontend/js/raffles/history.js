...
(function() {
    ...
    const historyModule = {
        ...
        init: function() {
            ...
            if (historyContainer) {
                ...
                exportButton.addEventListener('click', this.exportHistoryToCSV);
        ...
        getRafflesHistory: async function(filters = {}) {
            try {
                const now = Date.now();
                ...
                window.WinixRaffles.utils.showLoading('Завантаження історії розіграшів...');
                ...
                if (error.status === 500 || error.status === 404 ||
                    ...
                    if (_failedAttempts > 1) {
                        window.WinixRaffles.utils.showToast('Не вдалося завантажити історію розіграшів. Спробуйте пізніше.');
                    }
                ...
                window.WinixRaffles.utils.showToast('Не вдалося завантажити історію розіграшів');
        ...
        getRaffleHistoryDetails: async function(raffleId) {
            try {
                if (!raffleId) {
                ...
                if (!_historyData.find(item => item && item.raffle_id === raffleId)) {
                    window.WinixRaffles.utils.showToast('Не вдалося завантажити деталі розіграшу');
                }
        ...
        displayHistory: async function(containerId = 'history-container') {
            ...
            try {
                    ...
                    historyHTML += `
                        <div class="history-section">
                            ...
                            <div class="history-cards">
                                ${wonRaffles.map(this._createHistoryCardHTML).join('')}
                            </div>
                        </div>
                    `;
                }
                    ...
                    <div class="history-section">
                        ...
                        <div class="history-cards">
                            ${participatedRaffles.length > 0 
                                ? participatedRaffles.map(this._createHistoryCardHTML).join('')
                ...
                if (window.WinixRaffles.stats && typeof window.WinixRaffles.stats.updateHistoryStats === 'function') {
                    window.WinixRaffles.stats.updateHistoryStats(history);
                }
            } catch (error) {
        ...
        showRaffleHistoryDetails: function(raffleData) {
            if (!raffleData) {
        ...
        exportHistoryToCSV: function() {
            if (!_historyData || !Array.isArray(_historyData) || _historyData.length === 0) {
                window.WinixRaffles.utils.showToast('Немає даних для експорту');
                return;
            }
            ...
            try {
                ...
                window.WinixRaffles.utils.showToast('Історію розіграшів успішно експортовано');
            } catch (error) {
                console.error('Помилка експорту історії:', error);
                window.WinixRaffles.utils.showToast('Не вдалося експортувати історію');
            }
        },
        ...
        _addStatsGrid: function(container) {
        ...
        _applyHistoryFilters: function(containerId) {
            ...
            this.getRafflesHistory({
                ...
                window.WinixRaffles.utils.showToast('Помилка при застосуванні фільтрів');
            });
        },

        // Створення HTML для картки історії
        _createHistoryCardHTML: function(item) {
            if (!item) return '';
        ...
        _createEmptyHistoryHTML: function(customMessage) {
        ...
        _generateWinnersListHTML: function(winners) {
        ...
        _addHistoryCardEventListeners: function() {
            try {
                const self = this;
                document.querySelectorAll('.history-card').forEach(card => {
                    card.addEventListener('click', function() {
                        ...
                        if (historyItem) {
                            self.showRaffleHistoryDetails(historyItem);
                        } else {
                            // Якщо не знайдено в масиві, спробуємо отримати з API
                            self.getRaffleHistoryDetails(raffleId).then(details => {
                                if (details) {
                                    self.showRaffleHistoryDetails(details);
                                } else {
                                    window.WinixRaffles.utils.showToast('Не вдалося отримати деталі розіграшу');
                                }
                            }).catch(error => {
                                console.error("Помилка отримання деталей розіграшу:", error);
                                window.WinixRaffles.utils.showToast('Помилка отримання деталей розіграшу');
                            });
                        }
                    });
                });
        ...
        _getTotalPrizeAmount: function(wonRaffles) {
    ...
    // Експортуємо модуль
    window.WinixRaffles.history = historyModule;
})();