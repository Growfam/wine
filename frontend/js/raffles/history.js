...
(function() {
    ...
    if (typeof WinixRaffles === 'undefined') {
        console.error('❌ WinixRaffles не знайдено! Переконайтеся, що core.js підключено раніше history.js');
        return;
    }
    ...
    const history = {
        ...
        loadRaffleHistory: async function(forceRefresh = false) {
            ...
            const now = Date.now();
            if (!forceRefresh && now - this.lastUpdate < this.cacheInterval && this.historyData.length > 0) {
            ...
            try {
                ...
                if (response.status === 'success' && Array.isArray(response.data)) {
                    ...
                    try {
                        localStorage.setItem('winix_raffle_history', JSON.stringify({
                            timestamp: now,
