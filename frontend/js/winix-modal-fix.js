...
(function() {
    ...
    window.WinixModalSystem = {
        ...
        // Функція отримання максимального балансу користувача
        getMaxBalance: async function() {
            try {
                if (window.WinixAPI && window.WinixAPI.getBalance) {
                    // Через централізований API модуль
                    return new Promise((resolve) => {
                        window.WinixAPI.getBalance((error, result) => {
                            if (error) {
                                console.error("Помилка отримання балансу:", error);
                                // Запасний варіант
                                resolve(parseFloat(localStorage.getItem('userTokens') || '0'));
                            } else {
                                resolve(result.tokens || 0);
                            }
                        });
                    });
                } else if (window.WinixCore && window.WinixCore.Balance) {
                    // Через WinixCore
                    ...
                    // Запасний варіант через localStorage
                    return parseFloat(localStorage.getItem('userTokens') || '0');
                }
            } catch (e) {
    ...
    window.createInputModal = async function(title, onConfirm) {
        ...
        try {
            const balance = await WinixModalSystem.getMaxBalance();
            input.placeholder = `Введіть суму (макс: ${balance.toFixed(2)})`;

            // Додаємо атрибут для максимального значення
            input.max = balance;

            // Додаємо функціонал перевірки максимального значення
    ...
    // Інтеграція з системою ініціалізації WINIX
