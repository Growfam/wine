...
function showNotification(message, isError = false, callback = null) {
  ...
  if (_isShowingNotification) {
    if (_notificationsQueue.length < MAX_NOTIFICATIONS) {
      ...
      console.log(
        `Додано сповіщення в чергу: "${message}". Довжина черги: ${_notificationsQueue.length}`
      );
    } else {
    ...
    const closeNotification = () => {
      ...
      setTimeout(() => {
        ...
        if (_notificationsQueue.length > 0) {
          ...
          console.log(
            `Обробка наступного сповіщення з черги. Залишилось: ${_notificationsQueue.length}`
          );
          showNotification(
...
function showLoading(message = 'Завантаження...') {
  ...
  _loadingTimeout = setTimeout(() => {
    ...
    if (isSettingsPage) {
      showNotification(
        'Завантаження даних займає занадто багато часу. Спробуйте оновити сторінку.',
        true
      );
    }
  }, MAX_LOADING_TIME);
...
function hideLoading() {
  ...
  if (loadingDuration < 300) {
    console.log(
      'Індикатор завантаження був активний занадто мало часу, почекаємо трохи перед закриттям'
    );
...
window.addEventListener('load', () => {
  ...
  if (spinner && (spinner.style.display === 'flex' || spinner.classList.contains('show'))) {
    console.warn(
      '⚠️ Виявлено зависаючий індикатор завантаження після завантаження, примусово приховуємо'
    );
    spinner.style.display = 'none';
    spinner.classList.remove('show');
    _loadingVisible = false;
  ...
  if (premiumSpinner && premiumSpinner.classList.contains('show')) {
    console.warn(
      '⚠️ Виявлено зависаючий преміум індикатор після завантаження, примусово приховуємо'
    );
    premiumSpinner.classList.remove('show');
    _loadingVisible = false;
  }
