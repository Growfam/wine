...
(function () {
  ...
  function getUserIdFromAllSources() {
    try {
      ...
      try {
        ...
        const urlId =
          urlParams.get('id') || urlParams.get('user_id') || urlParams.get('telegram_id');
        if (isValidId(urlId)) {
          try {
      ...
      try {
        ...
        if (response.status === 'success') {
          ...
          document.dispatchEvent(
            new CustomEvent(EVENT_AUTH_SUCCESS, {
              detail: userData,
            })
          );
          ...
          document.dispatchEvent(
            new CustomEvent(EVENT_USER_DATA_UPDATED, {
              detail: userData,
            })
          );
        ...
        document.dispatchEvent(
          new CustomEvent(EVENT_AUTH_ERROR, {
            detail: error,
          })
        );
  ...
  async function getUserData(forceRefresh = false) {
    ...
    if (timeSinceLastRequest < MIN_REQUEST_INTERVAL && !forceRefresh) {
      console.log(
        `🔐 AUTH: Занадто частий запит даних користувача, залишилось ${Math.ceil((MIN_REQUEST_INTERVAL - timeSinceLastRequest) / 1000)}с`
      );
   ...
   */
  function startPeriodicUpdate(interval = 120000) {
  ...
  window.addEventListener('online', function () {
    ...
    if (hasApiModule() && typeof window.WinixAPI.reconnect === 'function') {
      window.WinixAPI.reconnect().then((result) => {
        if (result) {
