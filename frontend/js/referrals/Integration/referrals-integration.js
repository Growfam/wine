...
const getLevelRewards = async (userId) => {
  try {
    ...
    const { fetchLevelRewards } = await import('../fetchLevelRewards.js');
    ...
    showToast('Помилка при отриманні данних про винагороди', 'error');
  }
};
...
export const initReferralSystem = () => {
    console.log("🚀 Початок ініціалізації реферальної системи...");

    // Отримуємо ID користувача
    console.log("🔍 Отримуємо ID користувача...");
    const userId = getUserId();
    console.log("✅ Отримано ID користувача:", userId);
    appState.userId = userId;

    console.log("🔧 Налаштовуємо елементи UI...");
    // Елементи інтерфейсу
    setupUIElements();
    console.log("✅ Елементи UI налаштовано");

    console.log("🔗 Генеруємо реферальне посилання...");
    // Генеруємо реферальне посилання при ініціалізації
    getReferralLink(userId);

    console.log("💰 Отримуємо історію прямих бонусів...");
    // Отримуємо історію прямих бонусів
    getDirectBonusHistory(userId);

    console.log("📊 Отримуємо статистику рефералів...");
    // Отримуємо статистику рефералів
    getReferralStats(userId);

    console.log("💎 Отримуємо відсоткові винагороди...");
    // Отримуємо відсоткові винагороди (Етап 4)
    getLevelRewards(userId);

    console.log("📈 Отримуємо дані про активність рефералів...");
    // Отримуємо дані про активність рефералів (Етап 5)
    getReferralActivity(userId);

    console.log("🎯 Автоматично обробляємо реферальний параметр з URL...");
    // Автоматично обробляємо реферальний параметр з URL
    autoProcessReferral();

    console.log("🏆 Отримуємо дані про бейджі та завдання...");
    // Отримуємо дані про бейджі та завдання (Етап 6)
    getUserBadges(userId);
    getUserTasks(userId);

    console.log("🧭 Налаштовуємо вкладки...");
    // Налаштовуємо вкладки для структури рефералів
    setupReferralTabs();

    console.log("🔍 Налаштовуємо обробник деталей реферала...");
    // Налаштовуємо обробник для деталей реферала
    setupReferralDetails();

    console.log("🎁 Налаштовуємо обробники для бейджів та завдань...");
    // Налаштовуємо обробники для бейджів та завдань
    setupBadgesAndTasksHandlers();

    console.log("🎲 Отримуємо дані про участь у розіграшах...");
    // Отримуємо дані про участь у розіграшах (Етап 7)
    getReferralDrawsData(userId);
    getDrawsRanking(userId);

    console.log("🏅 Налаштовуємо обробники для рейтингу та аналітики...");
    // Налаштовуємо обробники для рейтингу та аналітики
    setupReferralRankingHandlers();

    console.log("💎 Отримуємо загальні дані про винагороди...");
    // Отримуємо загальні дані про винагороди (Етап 8)
    calculateTotalEarnings(userId);

    console.log("🎉 Ініціалізація реферальної системи завершена успішно!");
};