    ...
    function overrideRaffleDetailsModal() {
        ...
        window.createRaffleDetailsModal = function(raffleData) {
            ...
            try {
                ...
                console.error("❌ Помилка при створенні модального вікна розіграшу:", error);

                // Використовуємо централізовану обробку помилок, якщо доступна
                if (window.WinixAPI && window.WinixAPI.handleApiError) {
                    ...
                    if (window.showNotification) {
                        ...
                        window.simpleAlert(errorMessage, true);
                    } else {
                        alert(errorMessage);
                    }
                }
                ...
                if (originalCreateRaffleDetailsModal && typeof originalCreateRaffleDetailsModal === 'function') {
    ...
    function generatePremiumModalHTML(raffleData) {
        ...
        if ((!winners || winners.length === 0) && window.WinixAPI && window.WinixAPI.getRaffleWinners) {
            try {
                ...
                console.warn("Неможливо отримати список переможців, використовуємо штучні дані:", e);
                winners = generateRealWinners(isUsdtRaffle, isWinixRaffle, winnersCount, status);
            }
