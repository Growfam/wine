        ...
        window.WinixCore.updateUserDisplay = function () {
          try {
            ...
            try {
              ...
              console.warn(
                '⚠️ Помилка запасного методу оновлення даних користувача:',
                fallbackError
              );
            }
          }
        ...
        window.apiRequest = function (endpoint, method, data, options, retries) {
          try {
            ...
            if (endpoint.includes('/staking') && !endpoint.includes('/api/user/')) {
              const userId =
                window.WinixAPI.getUserId() || localStorage.getItem('telegram_user_id');
              if (userId) {
        ...
        window.WinixAPI.calculateExpectedReward = function (amount, period) {
          return new Promise((resolve) => {
            originalCalculateReward(amount, period)
              ...
              .catch((error) => {
                console.warn(
                  '⚠️ Помилка розрахунку винагороди через API, використовуємо локальний розрахунок:',
                  error
                );
        ...
        window.WinixStakingSystem.updateUI = function () {
          try {
            ...
            try {
              ...
              const stakingData =
                window.WinixStakingSystem.getStakingData() ||
                JSON.parse(
                  localStorage.getItem('stakingData') ||
              ...
              if (stakingStatus) {
                if (stakingData && stakingData.hasActiveStaking) {
                  safeUpdateContent(
                    stakingStatus,
                    `У стейкінгу: ${stakingData.stakingAmount || 0} $WINIX`
                  );
                } else {
              ...
              window.WinixStakingSystem.updateButtonsState(
                stakingData && stakingData.hasActiveStaking
              );
            } catch (fallbackError) {
              console.warn(
                '⚠️ Помилка запасного методу оновлення інтерфейсу стейкінгу:',
                fallbackError
              );
            }
          }
        ...
        window.WinixStakingSystem.updateButtonsState = function (hasActiveStaking) {
          try {
            ...
            try {
              ...
              console.warn(
                '⚠️ Помилка запасного методу оновлення стану кнопок стейкінгу:',
                fallbackError
              );
            }
          }
        ...
        try {
          ...
          if (key === 'userTokens' || key === 'winix_balance') {
            ...
            if (isNaN(numValue) || numValue < 0) {
              console.warn(
                `⚠️ Знайдено некоректне числове значення в localStorage[${key}]:`,
                value
              );
          ...
          if (key === 'userCoins') {
            ...
            if (isNaN(numValue) || numValue < 0) {
              console.warn(
                `⚠️ Знайдено некоректне цілочисельне значення в localStorage[${key}]:`,
                value
              );
              localStorage.setItem(key, '0');
            }
