...
(function () {
  ...
  function isOnline() {
    ...
    if (
      _state.networkState.reconnectionAttempts > 3 &&
  ...
  function classifyError(error) {
    ...
    // Перевіряємо, чи це помилка мережі
      ...
      (error.name === 'NetworkError' ||
        ...
        (error.message && (error.message.includes('network') || error.message.includes('Network'))))
    ) {
    ...
    try {
      ...
      if (error.message && error.message.includes('CORS')) {
        ...
        document.dispatchEvent(
          new CustomEvent('cors-issue-detected', {
  ...
  function generateUserFriendlyErrorMessage(error) {
    ...
    switch (classifiedError.type) {
      case 'cors':
        message =
          "Виникла проблема з доступом до сервера. Це може бути пов'язано з налаштуваннями безпеки.";
        break;
      case 'network':
  ...
  async function executeApiRequest(endpoint, method = 'GET', data = null, options = {}) {
    ...
    while (attempt < requestOptions.retries) {
      ...
      try {
        ...
        if (classifiedError.isCorsError) {
          console.error(
            `❌ Core: CORS помилка при виконанні ${method} ${endpoint}:`,
        ...
        if (classifiedError.isAuthError) {
          console.error(
            `❌ Core: Помилка авторизації при виконанні ${method} ${endpoint}:`,
            classifiedError.message
          );
          break;
        }
        ...
        if (classifiedError.isNetworkError || classifiedError.isTimeout) {
          console.warn(
            `⚠️ Core: ${classifiedError.type} помилка при виконанні ${method} ${endpoint} (спроба ${attempt}/${requestOptions.retries}):`,
            classifiedError.message
          );
        ...
        if (classifiedError.isServerError) {
          console.warn(
            `⚠️ Core: Помилка сервера при виконанні ${method} ${endpoint} (спроба ${attempt}/${requestOptions.retries}):`,
            classifiedError.message
          );
          continue;
        }
        ...
        console.error(
          `❌ Core: Помилка при виконанні ${method} ${endpoint}:`,
          classifiedError.message
        );
        break;
      }
    ...
    if (_state.errorCounter >= _state.maxErrorsBeforeReset && !options.preventReset) {
      console.warn(
        `⚠️ Core: Досягнуто критичної кількості помилок (${_state.errorCounter}), скидання стану...`
      );
  ...
  function updateUserDisplay() {
    try {
      ...
      const userId =
        userData.telegram_id || getUserId() || getFromStorage('telegram_user_id', 'Unknown ID');
  ...
  function getBalance() {
    try {
      return (
        ...
        parseFloat(getFromStorage('winix_balance', '0')) ||
        0
      );
    } catch (e) {
      return 0;
  ...
  function getCoins() {
    try {
      ...
      const storedCoins =
        parseInt(getFromStorage('userCoins', '0')) || parseInt(getFromStorage('winix_coins', '0'));
      if (storedCoins) {
        return storedCoins;
      }
  ...
  async function diagnoseBeckendConnection() {
    ...
    try {
      ...
      if (userId) {
        ...
        for (const endpoint of endpointsToCheck) {
          ...
          try {
            ...
            console.log(
              `${response.status === 'success' ? '✅' : '❌'} ${endpoint.name}: ${response.status}`
            );
          } catch (error) {
            results.apiEndpointStatus[endpoint.name] = {
              status: 'error',
      ...
      console.log(
        `🔄 Загальний статус підключення до бекенду: ${isWorking ? '✅ Працює' : '❌ Не працює'}`
      );
      ...
      if (results.corsIssues.hasCorsIssues) {
        results.recommendations.push(
          'Виявлено проблеми з CORS. Перевірте заголовки та налаштування сервера'
        );
      }

      if (!results.apiAvailable) {
        results.recommendations.push(
          'API модуль не доступний. Перевірте завантаження скрипту api.js'
        );
      }

      if (results.mockDataIndicatorPresent) {
        results.recommendations.push(
          'Додаток використовує тестові дані. Перевірте налаштування API'
        );
      }
  ...
  async function troubleshootConnection() {
    ...
    try {
      ...
      if (reconnected) {
        ...
        // Показуємо повідомлення користувачу
        showErrorMessage(
          "Не вдалося відновити з'єднання з сервером. Використовуються локальні дані.",
          'warning'
        );
      }
      ...
      saveToStorage(
        'connection_troubleshooting_results',
        JSON.stringify({
          timestamp: Date.now(),
          results,
        })
  ...
  async function syncUserData(forceRefresh = false) {
    try {
      ...
      if (!isOnline() && !forceRefresh) {
        ...
        if (dataStatus === 'stale') {
          showErrorMessage(
            'Використовуються локально збережені дані. Оновіть баланс при підключенні.',
            'info'
          );
        }
   ...
   */
  function startAutoSync(interval = 300000) {
    // 5 хвилин
    ...
    _state.refreshInterval = setInterval(async () => {
      try {
        ...
        if (
          Date.now() - _state.lastRequestTime >= _config.minRequestInterval &&
  ...
  async function init(options = {}) {
    try {
      ...
      console.log(
        `🔌 Core: Перевірка з'єднання з сервером: ${connectionStatus ? '✅ Успішно' : '❌ Помилка'}`
      );
      ...
      if (!connectionStatus && typeof navigator.onLine !== 'undefined' && navigator.onLine) {
        if (_config.detectCorsIssues) {
          ...
          if (corsIssues.hasCorsIssues) {
            ...
            showErrorMessage(
              'Виявлено проблеми з доступом до сервера. Використовуються локальні дані.',
              'warning'
            );
          }
        }
      ...
      if (_config.activeConnectionDiagnostics) {
        console.log(
          `🔄 Core: Запуск активної діагностики з'єднання (інтервал: ${_config.diagnosticsInterval}ms)`
        );
  ...
  window.addEventListener('online', function () {
    ...
    setTimeout(async () => {
      ...
      if (isConnected) {
        ...
        console.warn(
          "⚠️ Core: З'єднання з мережею відновлено, але з'єднання з сервером не встановлено"
        );
      }
    }, 1000);
  });
