/**
 * Task Progress API - модуль для роботи з прогресом завдань
 *
 * Відповідає за:
 * - Моніторинг прогресу завдання
 * - Аналіз стану завдань
 * - Сповіщення про зміни прогресу
 *
 * @version 3.0.0
 */
...
/**
 * API для роботи з прогресом завдань
 */
class TaskProgressApi {
    ...
    async getUserTasksProgress(options = {}) {
        try {
            ...
            // Формуємо URL для запиту
            const endpoint = CONFIG.API_PATHS.USER_PROGRESS(userId);
            ...
            return {
                status: 'error',
                message: 'Не вдалося отримати прогрес завдань',
                error: error.message
            };
        }
    }
    ...
    startProgressMonitoring(taskId, interval = 5000, callback) {
        ...
        const updateProgress = async () => {
            try {
                ...
                if (progress.status === 'success' && progress.data &&
    ...
    async analyzeTaskProgress(taskId, options = {}) {
        try {
            ...
            const analysis = {
                taskId,
                isStarted: !!status.started_at || status.status === 'in_progress' || status.status === 'completed',
...
export default new TaskProgressApi();