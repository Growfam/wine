...
export class RequestService {
  ...
  async request(endpoint, options = {}) {
    ...
    while (attempt <= requestOptions.maxRetries) {
      try {
        ...
        if (error.name === 'AbortError') {
          console.warn(
            `Таймаут запиту до ${endpoint} (спроба ${attempt}/${requestOptions.maxRetries + 1})`
          );
          continue;
        }
        ...
        console.warn(
          `Помилка запиту до ${endpoint} (спроба ${attempt}/${requestOptions.maxRetries + 1}):`,
          error.message
        );
      }
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, requestOptions.retryDelay * Math.pow(1.5, attempt - 1))
      );
    }
    ...
    return {
      ...
      errorCode:
        lastError?.name === 'AbortError' ? API_ERROR_CODES.TIMEOUT : API_ERROR_CODES.NETWORK_ERROR,
    };
  }
