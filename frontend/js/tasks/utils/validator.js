...
function injectStyles() {
    ...
    DOMUtils.injectStyles('validators-styles', css);
}
...
function setupDOMObserver() {
    // Використовуємо DOMUtils для спостереження за DOM
    DOMUtils.observeDOM(document.body, (mutations) => {
...
function setupFormValidation(form) {
    ...
    DOMUtils.addEvent(form, 'submit', submitHandler);
...
function setupFieldValidation(field) {
    ...
    if (config.liveValidation) {
        // Використовуємо DOMUtils для обробника з debounce
        DOMUtils.addEvent(field, 'input', () => {
            validateField(field);
...
function scrollToFirstError(form) {
    ...
    if (firstError) {
        // Використовуємо DOMUtils для прокрутки до елемента
        DOMUtils.scrollToElement(firstError, {
            behavior: 'smooth',
            block: 'center',
            offset: 50
        });
...
function validateDate(value, format) {
    ...
    if (format === 'yyyy-mm-dd' && parts.length === 3) {
        ...
        isValid = TimeUtils.isValidDate(year, month, day);
    }
...
// Експортуємо API
window.UI.Validators = Validators;
