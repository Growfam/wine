...
import { state } from 'js/tasks/ui/animations/core.js';
import { createSuccessParticles, highlightElement, pulseElement  } from 'js/tasks/ui/animations/effects/index.js';
  ...
  try {
    ...
    setTimeout(() => {
      taskElement.dispatchEvent(
        new CustomEvent('completion-animation-ended', {
          bubbles: true,
          detail: { taskId },
        })
      );
...
export function animateTaskError(taskId, options = {}) {
  ...
  if (!taskElement) {
    logger.warn(
      `Не знайдено елемент завдання для анімації помилки: ${taskId}`,
      'animateTaskError',
      {
        category: LOG_CATEGORIES.ANIMATION,
      }
...
export function animateTaskExpiration(taskId, options = {}) {
  ...
  if (!taskElement) {
    logger.warn(
      `Не знайдено елемент завдання для анімації закінчення: ${taskId}`,
...
export { animateTaskCompletion, animateTaskError, animateTaskExpiration, animateTaskStart };
