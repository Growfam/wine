...
export function showProgressAnimation(taskId, progress, options = {}) {
    ...
    if (!progressBar) {
        logger.warn(`Не знайдено елемент прогрес-бару для завдання: ${taskId}`, 'showProgressAnimation', {
            category: LOG_CATEGORIES.ANIMATION
        });
        return false;
    }
    ...
    try {
        ...
        logger.info(`Анімація прогресу завдання ${taskId}: ${currentWidth}% -> ${normalizedProgress}%`, 'showProgressAnimation', {
            category: LOG_CATEGORIES.ANIMATION,
...
export function animateTasksFiltering(container, filterFunc, options = {}) {
    ...
    try {
        ...
        setTimeout(() => {
            container.dispatchEvent(new CustomEvent('tasks-filtering-complete', {
                bubbles: true,
                detail: {
