...
10,
        ...
        logger.debug(`Створення ${settings.count} часток для анімації успіху`, 'createSuccessParticles', {
...
function createSingleParticle(centerX, centerY, settings) {
    try {
        ...
        particle.style.borderRadius = Math.random() > 0.5 ? '50%' : `${Math.floor(Math.random() * 4) + 2}px`;
...
function createConfettiParticle(container, settings) {
    try {
        ...
        const targetY = startY + Math.sin(angleXY) * settings.spread.y * (Math.random() * 0.7 + 0.3) + settings.spread.y * 2;
...
export function createStarsEffect(targetElement, options = {}) {
    try {
        ...
        } else if (typeof targetElement === 'object' && targetElement.x !== undefined && targetElement.y !== undefined) {
