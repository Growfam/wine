...
// Імпорт залежностей відповідно до структури проекту
import { TaskManager } from '../../services/task-store.js';
...
function fillTaskContent(taskElement, task, progress, options) {
    ...
    if (task.target_value > 1) {
        ...
        if (progressContainer && TaskProgress) {
            TaskProgress.render(progressContainer, task, progress);
        }
    }
...
function setupTaskActions(taskElement, task, progress, options) {
    ...
    if (needsVerification) {
        ...
        verifyBtn.addEventListener('click', function(event) {
            ...
            if (TaskManager && TaskManager.verifyTask) {
                TaskManager.verifyTask(task.id);
            }
        });
    ...
    // Інакше додаємо кнопку "Виконати" / "Почати"
        ...
        startBtn.addEventListener('click', function(event) {
            event.preventDefault();
            if (TaskManager && TaskManager.startTask) {
