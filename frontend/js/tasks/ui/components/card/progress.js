...
function createProgressBar(element, percent, config) {
  ...
  if (config.showText) {
    ...
    const text =
      typeof config.format === 'function' ? config.format(percent) : `${Math.round(percent)}%`;
...
export function updateProgress(taskId, progress, options = {}) {
  ...
  if (isSteps) {
    ...
    updateStepsProgress(
      element,
      progress,
      updatedConfig.maxValue ||
        parseInt(element.querySelector('.progress-text')?.textContent.split('/')[1]) ||
        100,
      updatedConfig
    );
  } else {
