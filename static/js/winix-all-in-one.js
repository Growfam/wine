...
(function() {
    ...
    // ===== ПОВНЕ ВИПРАВЛЕННЯ UI ДЛЯ СИСТЕМНИХ ВІКОН =====
// Замінює всі функції створення вікон і забезпечує єдиний стиль
// Додайте цей код в початок файлу winix-all-in-one.js

(function() {
    console.log("🔄 Застосування виправлення UI системних вікон...");

    // Зберігаємо посилання на оригінальні функції
    const originalCreateInputModal = window.createInputModal;
    const originalSimpleAlert = window.simpleAlert;
    const originalShowNotification = window.showNotification;

    // Змінна для відстеження відкритих вікон
    let isModalOpen = false;
    let isAlertOpen = false;
    let isNotificationOpen = false;

    // CSS стилі для всіх системних вікон
    const uiStyles = `
    /* Стилі для модальних вікон і сповіщень */
    .winix-modal-overlay {
        position: fixed;
        top: 0;
        left: 0;
    ...
    function addStyles() {
        if (!document.getElementById('winix-ui-styles')) {
            const styleElement = document.createElement('style');
            styleElement.id = 'winix-ui-styles';
            styleElement.textContent = uiStyles;
            document.head.appendChild(styleElement);
            console.log("✅ Стилі UI системних вікон додано");
        }
    }

    // Функція для видалення всіх модальних вікон
    function removeAllModals() {
        const modals = document.querySelectorAll('.winix-modal-overlay, .modal-overlay, .alert-overlay');
    ...
    // Функція для видалення всіх сповіщень
    function removeAllNotifications() {
        const notifications = document.querySelectorAll('.winix-notification, .notification, .toast-message.show');
    ...
    // Нова функція для створення модального вікна з полем введення
    window.createInputModal = function(title, onConfirm) {
        ...
        container.className = 'winix-modal-container';

        const modalTitle = document.createElement('div');
        modalTitle.className = 'winix-modal-title';
        modalTitle.textContent = title || 'Введіть суму:';

        const inputContainer = document.createElement('div');
        inputContainer.className = 'winix-input-container';

        const input = document.createElement('input');
        input.className = 'winix-modal-input';
        input.type = 'number';
        input.min = '0';
        input.step = 'any';
        ...
        buttonContainer.className = 'winix-modal-buttons';

        const cancelButton = document.createElement('button');
        cancelButton.className = 'winix-modal-button cancel';
        cancelButton.textContent = 'Скасувати';

        const confirmButton = document.createElement('button');
        confirmButton.className = 'winix-modal-button confirm';
        ...
        // Встановлюємо флаг відкритого модального вікна
        isModalOpen = true;
    ...
    // Нова функція для показу повідомлень
    window.simpleAlert = window.winixUI.simpleAlert = function(message, isError = false, callback) {
        console.log("📣 Показ повідомлення:", message, isError ? "(помилка)" : "");
        ...
        return new Promise((resolve) => {
            ...
            const container = document.createElement('div');
            container.className = 'winix-modal-container' + (isError ? ' error' : '');

            const messageElement = document.createElement('div');
            messageElement.className = 'winix-modal-message';
            ...
            // Встановлюємо флаг відкритого повідомлення
            isAlertOpen = true;
    ...
    window.showNotification = function(message, type = 'info', duration = 3000) {
        console.log("📣 Показ сповіщення:", message, type);
        ...
        // Встановлюємо флаг відкритого сповіщення
    ...
    // Заміна обробників кнопок при завантаженні
        ...
        if (addButton) {
            ...
            newAddButton.addEventListener('click', function() {
                window.createInputModal('Введіть суму для додавання до стейкінгу:', function(amount) {
                    console.log("Додавання до стейкінгу:", amount);
            ...
            console.log("✅ Обробник кнопки додавання до стейкінгу замінено");
        }
    });

    // Викликаємо додавання стилів відразу
    addStyles();

    console.log("✅ Виправлення UI системних вікон застосовано");
})();
    // ====================== ДОПОМІЖНІ ФУНКЦІЇ ======================
